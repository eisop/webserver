...
public class JsonPrinter extends Printer {
    ...
    static JsonObjectBuilder buildException(String msg) {
        return Json.createObjectBuilder().add("type", "exception").add("msg", msg);
    }
    ...
    public void printSuccess() {
        ...
        assert this.execCmd != null
                : "a success info should given based on non-null execute command";
    ...
    public void printDiagnosticReport(List<Diagnostic<? extends JavaFileObject>> diagnosticList) {
        assert this.usercode != null
                : "a diagnostic report should given based on non-null usercode";
        assert this.execCmd != null
                : "a diagnostic report should given based on non-null execute command";
        ...
        for (Diagnostic<? extends JavaFileObject> err : diagnosticList) {
            ...
            JsonObjectBuilder errorBuilder =
                    JsonPrinter.buildCompileInfo(
                            errHeader + err.getMessage(null),
                            errType,
        ...
        assert errorReport.size() > 0
                : "bytecode is null or diagnosticList is not empty, but error report is empty.";
