...
public class CompileToBytes {
    ...
    // a diagnostic (warning, etc) listener; if null use the compiler's default method for reporting
    // diagnostics
    ...
    /**
     * Compiles a single source file to bytecode. Returns null if compilation failed (same as
     * JavaCompiler.getTask.call => false). Otherwise, returns bytecode for files defined as a
     * result of compiling.
     */
    ...
    public Map<String, byte[]> compileFiles(String[][] classCodePairs) {
        if (used) {
            throw new RuntimeException("You already used this CompileToBytes.");
        }
        used = true;
        ...
        RAMClassFileManager fileManager =
                new RAMClassFileManager(compiler.getStandardFileManager(null, null, null));
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        job.add("status", "Success").add("bytecodes", classFiles.build());
        if (jerr != null) {
            job.add("warning", jerr);
        }
